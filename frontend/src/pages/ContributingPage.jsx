...
import {
  GitFork, GitPullRequest, ShieldCheck, BookOpen, Terminal, Heart,
  CheckCircle2, ChevronRight, Lightbulb, MessageSquare, Zap, Code2,
  Bug, Star, ArrowRight, AlertTriangle, Layers, TestTube2, Rocket
} from 'lucide-react';
...
const GOOD_FIRST_ISSUES = [
  ...
  { title: 'Implement rate-limit retry logic in API client', type: 'Backend', difficulty: 'Medium' },
  ...
  return (
    <section id={id} className="scroll-mt-24 mb-16">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 id={id} className="text-xl md:text-2xl font-black text-white">{title}</h2>
      </div>
      {children}
    </section>
  );
...
function InfoBox({ type = 'info', children }) {
  const styles = {
    ...
    warn:    { bg: 'bg-amber-500/8  border-amber-500/25 ', text: 'text-amber-400',   icon: AlertTriangle },
    success: { bg: 'bg-emerald-500/8 border-emerald-500/25', text: 'text-emerald-400', icon: CheckCircle2 },
  };
  const s = styles[type];
  return (
    ...
    <SidebarPageLayout title="Contributing" sidebarLinks={sidebarLinks}>
      ...
      <div className="mb-12">
        ...
        <p className="text-slate-400 text-lg md:text-xl max-w-2xl mb-8 leading-relaxed">
          Whether it's a bug fix, a new integration, or an improvement to the docs — every contribution matters. Here's everything you need to get started.
        </p>
      ...
      <Section id="welcome" icon={Heart} title="Welcome, Contributor!" iconColor="text-pink-400">
        <div className="bg-[#0d1117] border border-white/8 rounded-2xl p-7 mb-6">
          <p className="text-slate-400 leading-relaxed mb-4">
            DevPulse is an open-source, AI-powered DevSecOps platform. We believe that security and developer experience should go hand-in-hand — and we're building that in public with the community.
          </p>
          <p className="text-slate-400 leading-relaxed">
            Contributions of any size are welcome: typo fixes, new features, bug reports, documentation improvements, or even just sharing the project. Thank you for being here. 🙌
          </p>
        </div>
        <InfoBox type="success">
          <strong>New here?</strong> Start with a "good first issue" — we've labeled them specifically so you can hit the ground running without needing deep context.
        </InfoBox>
      </Section>

      {/* Code of Conduct */}
      <Section id="code-of-conduct" icon={ShieldCheck} title="Code of Conduct" iconColor="text-purple-400">
        <div className="bg-[#0d1117] border border-white/8 rounded-2xl p-7 space-y-4">
          <p className="text-slate-400 leading-relaxed">
            We are committed to providing a welcoming and inclusive environment for everyone. By participating, you agree to uphold the following:
          </p>
          {[
      ...
      {/* Getting Started */}
      <Section id="getting-started" icon={Terminal} title="Getting Started" iconColor="text-blue-400">
        <InfoBox type="info">
          You'll need <strong>Node.js 20+</strong>, <strong>Python 3.11+</strong>, <strong>Docker</strong>, and <strong>Git</strong> installed before you begin.
        </InfoBox>
...
cp frontend/.env.example frontend/.env`}</CodeBlock>
            Fill in the required values — at minimum <Code>DATABASE_URL</Code>, <Code>REDIS_URL</Code>, and a GitHub OAuth app.
          </Step>
...
git checkout -b fix/issue-description`}</CodeBlock>
      ...
      {/* How to Contribute */}
      <Section id="how-to-contribute" icon={Layers} title="How to Contribute" iconColor="text-orange-400">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-8">
          {[
            { icon: Bug,          color: 'text-red-400',    bg: 'bg-red-500/10 border-red-500/20',       title: 'Fix Bugs',            desc: 'Browse open issues tagged "bug" and submit a PR with a fix and a regression test.' },
            { icon: Lightbulb,    color: 'text-blue-400',   bg: 'bg-blue-500/10 border-blue-500/20',     title: 'Build Features',       desc: 'Pick up a planned roadmap item or propose your own. Open an issue first to discuss.' },
            { icon: BookOpen,     color: 'text-purple-400', bg: 'bg-purple-500/10 border-purple-500/20', title: 'Improve Docs',         desc: 'Find something confusing or missing in the docs? We love clear, accurate documentation.' },
            { icon: TestTube2,    color: 'text-teal-400',   bg: 'bg-teal-500/10 border-teal-500/20',     title: 'Write Tests',          desc: 'Add unit, integration, or E2E tests for existing features. Coverage improvements are always welcome.' },
            { icon: Zap,          color: 'text-amber-400',  bg: 'bg-amber-500/10 border-amber-500/20',   title: 'Performance',          desc: 'Found a slow query or a large bundle? Profile it, prove the improvement, and send the PR.' },
            { icon: MessageSquare,color: 'text-emerald-400',bg: 'bg-emerald-500/10 border-emerald-500/20',title: 'Community Support',  desc: 'Help answer questions in GitHub Discussions or Discord. Supporting others is contributing too.' },
          ].map((item, i) => (
            <div key={i} className={`bg-[#0d1117] border ${item.bg} rounded-2xl p-5 hover:bg-white/[0.03] transition-colors`}>
        ...
        <div className="bg-[#0d1117] border border-white/8 rounded-2xl overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-white/8">
            ...
            <a href="https://github.com/SSSahil15/DevPulse/issues?q=label%3A%22good+first+issue%22" target="_blank" rel="noreferrer"
          ...
          <div className="divide-y divide-white/5">
            {GOOD_FIRST_ISSUES.map((issue, i) => (
              <div key={i} className="px-6 py-4 flex items-center justify-between gap-4 hover:bg-white/[0.03] transition-colors cursor-pointer group">
                <span className="text-sm text-slate-300 group-hover:text-white transition-colors">{issue.title}</span>
                <div className="flex items-center gap-2 shrink-0">
                  <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${TYPE_COLORS[issue.type]}`}>{issue.type}</span>
                  <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${DIFF_COLORS[issue.difficulty]}`}>{issue.difficulty}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <Section id="pull-requests" icon={GitPullRequest} title="Pull Request Process" iconColor="text-emerald-400">
        <div className="bg-[#0d1117] border border-white/8 rounded-2xl p-7 space-y-1 mb-5">
          <Step num="1" title="Keep PRs focused">
            One feature or fix per PR. Large, multi-concern PRs are hard to review and slower to merge.
          </Step>
          <Step num="2" title="Fill out the PR template">
            Describe <em>what</em> changed and <em>why</em>. Link the related issue with <Code>Closes #123</Code>.
          </Step>
...
cd backend  && npm run lint`}</CodeBlock>
          ...
          <Step num="5" title="Request a review">
            Assign at least one maintainer as a reviewer. Address feedback constructively — we iterate together.
          </Step>
        ...
        <InfoBox type="warn">
          PRs that add features without tests, or that break existing tests, will not be merged until fixed.
        </InfoBox>
      </Section>
      ...
      <Section id="reporting-bugs" icon={Bug} title="Reporting Bugs" iconColor="text-red-400">
        <div className="bg-[#0d1117] border border-white/8 rounded-2xl p-7">
          <p className="text-slate-400 text-sm leading-relaxed mb-5">
            A good bug report means we can reproduce and fix it faster. Include the following in every report:
          </p>
          {[
            { label: 'Environment',       desc: 'OS, browser, Node/Python version, Docker version.' },
            { label: 'Steps to Reproduce', desc: 'A numbered, minimal list of actions that triggers the bug.' },
            { label: 'Expected Behaviour', desc: 'What should have happened.' },
            { label: 'Actual Behaviour',   desc: 'What actually happened — include error messages and stack traces.' },
            { label: 'Screenshots / Logs', desc: 'Attach relevant logs from Docker, the console, or Sentry if available.' },
          ].map((field, i) => (
          ...
          <div className="mt-5">
            <a href="/community" className="inline-flex items-center gap-2 text-sm font-bold text-red-400 hover:text-red-300 transition-colors">
      ...
      {/* Feature Requests */}
      <Section id="feature-requests" icon={Lightbulb} title="Feature Requests" iconColor="text-amber-400">
        <div className="bg-[#0d1117] border border-white/8 rounded-2xl p-7">
          <p className="text-slate-400 text-sm leading-relaxed mb-5">
            Before submitting a feature request, check the <a href="/roadmap" className="text-blue-400 hover:underline">Public Roadmap</a> to see if it's already planned. If not, we'd love to hear it.
          </p>
          <p className="text-slate-400 text-sm leading-relaxed mb-6">
            Great feature requests include: a clear problem statement, who is affected, and a proposed solution. Bonus points for mockups or user stories.
          </p>
          <a href="/community" className="inline-flex items-center gap-2 text-sm font-bold text-amber-400 hover:text-amber-300 transition-colors">
      ...
      <Section id="recognition" icon={Star} title="Recognition" iconColor="text-amber-400">
        <div className="bg-gradient-to-br from-[#0d1117] via-[#0d1117] to-blue-950/30 border border-blue-500/20 rounded-2xl p-7">
          ...
          {[
            ...
            { icon: Star,          text: 'Significant contributors earn a special role in the Discord server.' },
            { icon: CheckCircle2,  text: 'Core contributors are invited to the private maintainers channel.' },
