...
export default function ChangelogPage() {
  ...
  return (
    <SidebarPageLayout
      title="Changelog"
      sidebarLinks={RELEASES.map((r) => ({ id: r.version, title: r.version, icon: Sparkles }))}
    >
      <div className="mb-12">
        ...
        <p className="text-slate-400 text-lg md:text-xl max-w-2xl">
          See what's new in DevPulse. We ship improvements every week to make your DevSecOps
          seamless.
        </p>
      </div>
        ...
        {filteredReleases.length === 0 ? (
          ...
          filteredReleases.map((release, index) => (
            <section
              id={release.version}
              key={release.version}
              className="scroll-mt-24 relative flex flex-col md:flex-row gap-6 md:gap-12 group animate-in slide-in-from-bottom-4 duration-500"
              ...
              <div className="flex-1 bg-[#0d1117] border border-white/5 group-hover:border-white/10 rounded-2xl p-6 md:p-8 transition-colors shadow-xl">
                ...
                <ul className="space-y-4">
                  {release.updates.map((update, i) => (
                    <li
                      key={i}
                      className="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4 text-slate-300"
                    >
                      <span
                        className={`shrink-0 inline-flex items-center justify-center border text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider w-fit sm:mt-0.5 ${BADGE_COLORS[update.type] || 'bg-white/5 text-slate-400 border-white/10'}`}
                      >
                        {update.type}
                      </span>
