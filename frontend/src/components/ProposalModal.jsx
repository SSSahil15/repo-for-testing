...
export default function ProposalModal({ onClose }) {
  const [form, setForm]   = useState({ name: '', email: '', title: '', description: '', useCase: '' });
  ...
  const handleSubmit = async e => {
    ...
    try {
      await sendToDiscord({
        ...
        color:  0x3b82f6,
        fields: [
          { name: '👤 Submitted by', value: `${form.name}${form.email ? ` (${form.email})` : ''}`, inline: false },
    ...
    <ModalShell onClose={onClose}>
      {status === 'success' ? (
        <SuccessState
          ...
          onClose={onClose} accentColor="blue"
        />
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          <ModalHeader icon={<Terminal className="w-5 h-5 text-blue-400" />} title="Submit a Feature Proposal" subtitle="Great features start with great ideas." onClose={onClose} />
          ...
          <Field label="Feature Title" name="title" value={form.title} onChange={handleChange} placeholder="e.g. AI-powered dependency auto-updates" required />
          <TextareaField label="Description" name="description" value={form.description} onChange={handleChange} placeholder="What should this feature do?" required rows={4} />
          <TextareaField label="Use Case / Motivation" name="useCase" value={form.useCase} onChange={handleChange} placeholder="Why do you need this? What problem does it solve?" rows={3} />
          {status === 'error' && <ErrorBanner message={errorMsg} />}
          <SubmitButton loading={status === 'loading'} label="Submit Proposal" color="bg-blue-600 hover:bg-blue-500 shadow-blue-500/20" />
        </form>
      )}
    </ModalShell>
  );
