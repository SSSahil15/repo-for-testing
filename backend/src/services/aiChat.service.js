...
async function callPrimaryLLM(query, contextData, conversationHistory) {
  ...
  const buildStatus = (stages.backend?.build === "failure" || stages.frontend?.build === "failure" || stages.docker?.build === "failure") ? "failure" : "success";

  // Detailed vulnerability context
  let vulnsText = "None";
  if (security.vulnerabilities && security.vulnerabilities.length > 0) {
    const topVulns = security.vulnerabilities.slice(0, 5).map(v => 
      `${v.severity} in ${v.pkgName} (${v.id}) - Fix: ${v.fixedVersion || 'None'}`
    );
...
function callFallbackEngine(query, pipelineData, analysisResult, conversationHistory) {
  ...
  const hasVulnerabilities = (security.critical || 0) + (security.high || 0) + (security.medium || 0) > 0;
  const hasBuildFailures = stages.backend?.tests === "failure" || stages.frontend?.build === "failure" || stages.docker?.build === "failure";

  const isFollowUp = conversationHistory && conversationHistory.length > 2;

  // Default structure
  const result = {
    issue: isFollowUp ? "Context limited for follow-up question." : "Analysis based on high-level numeric metrics.",
  ...
  if (q.includes("score") || q.includes("health")) {
    if (score < 80) {
      if (hasVulnerabilities) {
        ...
        result.fix = "- Review package dependency tree\n- Apply necessary version upgrades starting with critical severity items.";
        result.explanation = "Unpatched dependencies introduce known CVEs, exposing the software to immediate risk.";
        result.risk = "HIGH";
        ...
        result.fix = "- Investigate failing CI/CD stages (backend tests or docker builds)\n- Revert breaking changes or fix broken tests.";
        result.explanation = "Failed pipeline stages prevent safe deployment and degrade integration health.";
        result.risk = "HIGH";
        ...
        result.explanation = "High churn and test instability decrease overall project reliability.";
        result.risk = "MEDIUM";
      ...
      result.explanation = "All core integration and security metrics are within acceptable thresholds.";
      result.risk = "LOW";
    }
  } else if (q.includes("fail") || q.includes("error")) {
    if (hasBuildFailures) {
      result.issue = "Recent pipeline execution failed at a critical CI/CD stage.";
      result.fix = "- Isolate the exact failing stage in logs\n- If it's a test suite, run locally to reproduce.";
      result.explanation = "Non-zero exit codes block the deployment pipeline and indicate broken integrations.";
      result.risk = "HIGH";
      result.suggestedActions = ["View detailed report", "Explain further"];
    } else {
      result.issue = `Pipeline hasn't failed, but there is a ${Math.round(failureProb)}% predicted risk of future failure.`;
      result.fix = "- Review potentially flaky tests\n- Monitor the next few integration cycles closely.";
      result.explanation = "Predictive analysis indicates elevated risk based on historical commit patterns and code churn.";
      result.risk = "MEDIUM";
      result.suggestedActions = ["Explain further"];
    }
    ...
    if (hasVulnerabilities) {
      ...
      if (topVuln) {
        ...
        result.fix = "- Run a standard security audit (e.g., 'npm audit')\n- Bulk-patch minor version bumps.";
        result.explanation = "Multiple dependencies contain known CVEs, increasing your software supply chain risk.";
      }
      result.risk = "HIGH";
      result.suggestedActions = ["Fix vulnerabilities", "Explain further"];
    } else {
